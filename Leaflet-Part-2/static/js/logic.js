// All earthquakes in past 30 days dataset
...
function createMaps(earthquake) {
  ...
  for (var i = 0; i < earthquake.length; i++) {
    ...
    }).bindPopup(`<h1>${earthquake[i].properties.place}</h1> <hr> <h3>Magnitude: ${earthquake[i].properties.mag} &emsp; Depth: ${earthquake[i].geometry.coordinates[2]}</h3>`).addTo(myMap);
  }
